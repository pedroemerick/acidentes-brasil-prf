...
#     padrao_lat = lat in range ((int(geo.lat) - 10), (int(geo.lat) + 10))
#     padrao_lng = lng in range ((int(geo.lng) - 40), (int(geo.lng) + 40))
#     return not padrao_lat or not padrao_lng

# br = geocoder.arcgis("Brasil")

# fora_padrao = []
# for index, row in datatran.iterrows():
#     lat = int(row.latitude)
#     lng = int(row.longitude)
#     if esta_no_padrao(br, lat, lng):
#         fora_padrao.append(index)

# datatran.drop(fora_padrao, inplace=True)
